...
* The challenge is finding the right level of abstraction for a given problem
...
* The following is *very* slow. Experienced R users will wince at the code.
...
* Avoiding loops leads to cleaner, more "R-ish" code.
* Some may find this less transparent and it is faster, but not by
  much.
...
* Contrast `save` and `load` with `saveRDS(compress=FALSE)` and
  `readRDS`
...
* Manipulate data with echoes of SQL. The code focuses on the task
...
* SQL databases usually have their own passwords
