...
Your data in R will typically reside in one or more dataframes, which
are rectangular data containers, like a spreadsheet worksheet.  A
given dataframe can store multiple types of data at once, subject to
the restriction that any given column only contains data of one type
(numeric, character, logical, etc.).  When performing your analysis,
you will undertake basic data operations, which can include extracting
...
manipulation of a dataframe. The purpose of this note is to illustrate the
various methods and explain which are equivalent. 
...
the dataframe or tibble in which they reside.^[Dataframes and tibbles
are both lists, which is why they can contain columns of different
vector classes.]

Of course you can *subset* multiple columns at once, because you are
retaining the dataframe or tibble from which subset.
...
The last example illustrates that subsetting and summarizing
